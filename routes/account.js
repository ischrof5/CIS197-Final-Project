...
router.get('/all_spaces', function (req, res) {
  //res.render('all_spaces');
  User.findOne({ username: req.session.user}, function(err, output) {
  	if (!err && output !== null) {
      arr_all_spaces = output.all_spaces;
    		res.render('all_spaces', {all_spaces: arr_all_spaces});
  	}
  })
})
